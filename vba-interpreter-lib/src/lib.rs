...
pub mod prepare {
  ...
  fn load_module(file: &File)-> crate::runtime::Module {
    let module = crate::gen::parse(&file.content.as_str());
    let module_type =
    ...
    build_module(file.base_name.as_str(), module_type, module)
  }
  
  pub fn load_modules(files: Vec<&File>, global_env: &mut Env) {
    for file in files {      
  ...
  pub fn load_and_run(files: Vec<&File>)-> (String, String) {
    let mut global_env = prepare_global_env();
    
    // extract base name of files[0]
    let entry_module = files[0].base_name.clone();
    load_modules(files, &mut global_env);
    
    crate::static_checker::check(&global_env);
    
    let prog = crate::runtime::Program::new();
    let (result, env) = prog.evaluate_program(&mut global_env, Rc::new(entry_module), Rc::new("main".to_owned()));
